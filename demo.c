...
int main(void)
{
    ...
    while(1) {
		...
		if (Demo.fDmaS2MMEvent)
		{
			...
			Xil_DCacheInvalidateRange(addr, rate*5*NR_AUDIO_SAMPLES);
...
//					//microblaze_flush_dcache();
			Xil_DCacheFlushRange(addr, rate*5*NR_AUDIO_SAMPLES);
			...
			switch(Demo.chBtn) {
				case 'u':
					if (!Demo.fAudioRecord && !Demo.fAudioPlayback)
					{
						...
						fnAudioRecord(sAxiDma,rate*NR_AUDIO_SAMPLES, addr);
						Demo.fAudioRecord = 1;
					}
					else
					{
				...
				case 'd':
					if (!Demo.fAudioRecord && !Demo.fAudioPlayback)
					{
						...
						xil_printf("\r\nStart Playback...\r\n");
						fnSetHpOutput();
						fnAudioPlay(sAxiDma,rate*NR_AUDIO_SAMPLES, addr);
				...
				case 'c':
					if(rate == 1){
						rate = 2;
						xil_printf("\r\Change Duration to 10s\r\n");
					}
					else{
						rate = 1;
						xil_printf("\r\Change Duration to 5s\r\n");
					}
				default:
					break;
			}
